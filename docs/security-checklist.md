...
## What "sensitive" means in this domain (pet boarding platform)

Treat all of the following as PII-tier data, subject to every rule below:

- Pets' names, photos, or any identifying images
...
## Owner-consent / pet-data handling

Applies even to synthetic/example data, as a demonstration of awareness:

- No pet data is used for AI training or fine-tuning
- No behavioral profiling of pets (usage patterns, engagement scoring)
- Retention limits are documented for any pet-related record, even if not
  enforced by a background job in this template itself
- Owner-consent language is noted in the relevant model/feature comment
  even where the example app doesn't implement a real consent flow
