...
class DailyReportTest < ActiveSupport::TestCase
  ...
  test "one report per pet per day" do
    existing = create(:daily_report)
    duplicate = build(:daily_report, pet: existing.pet, reported_on: existing.reported_on)
  ...
  test "saving a report refreshes everyone watching the pet" do
    pet = create(:pet)

    assert_turbo_stream_broadcasts pet, count: 1 do
      perform_enqueued_jobs { create(:daily_report, pet: pet) }
    end
  end
end
