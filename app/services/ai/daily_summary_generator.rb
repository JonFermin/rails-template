...
module Ai
  # Turns a DailyReport into an ActivitySummary for the pet's owners.
  #
  # Data about a pet leaves the application here, so the prompt is built from an explicit allowlist and nothing
  # else: no name, no ids, no photos, and not the attendant's free-text notes. Add a field to PROMPT_FIELDS only with
  # a second reviewer (docs/security-checklist.md → AI-specific rules, owner-consent / pet-data handling).
  class DailySummaryGenerator
    PROMPT_FIELDS = %i[ mood nap_minutes meals ].freeze

    SYSTEM_PROMPT = <<~TEXT
      You write a short, warm end-of-day recap for a pet's owners from structured notes an attendant recorded.
      Do not invent details, do not guess the pet's breed or age, and do not give medical or veterinary advice.
